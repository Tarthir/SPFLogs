...
class Test23(BaseClass.TestBase):
    ...
    def check_testing(self, log_list):
        ...
        for log in log_list:
            ...
            level = log.level
            if level is not None and "mail" in level:
                rec = level + ":" + rec
        ...
        if first_txt_time is not None:
            ...
            for entry in log_list:
                rec = entry.rec_queried
                rec = rec.strip()
                level = entry.level
                print(rec)
                if rec == "TXT" or rec == "SPF":
                    pass
                else:
                    bg_value = "BG_no_txt"
                    break
    # Append the test results file
        ...
        f = open("./validation_results/t23_results.txt", "a+")     # this will append to the t03 results file
        f.write(result)
        f.flush()
        f.close()
